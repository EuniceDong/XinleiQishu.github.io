...
function displayItems() {
  ...
  const AdultData = foodItem.items.filter((item) => item.category == "Adult");
  const KidData = foodItem.items.filter((item) => item.category == "Kid");
  const DrinkData = foodItem.items.filter((item) => item.category == "Drink");
  const ComboData = foodItem.items.filter((item) => item.category == "Combo");
...
const vegData = [
  ...new Map(foodItem.items.map((item) => [item["category"], item])).values(),
];
console.log(vegData);
...
function addToCart() {
  ...
  var itemObj = foodItem.items.find((element) => element.name == itemToAdd);
...
//  clickCardCarJumpButton
let cardBtn = document.querySelector(".cart-btn");
cardBtn.onclick = function () {
  let param = JSON.stringify(cartData);
  window.sessionStorage.setItem("tempCartData", param);
